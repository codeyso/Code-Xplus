...
def getdata(baseurl, max_keys, csv_filename, child_tags, marker='', page=0):
    ...
    # 写入数据
    nums, is_repeate, repeate_nums, total_nums = write_csv_content(baseurl, csv_filename, datas, has_namespace, namespace,
                                                                   child_tags)
    page += 1
...
def write_csv_content(baseUrl, csv_filename, datas, has_namespace, namespace, child_tags):
    # 提取数据并写入CSV文件
